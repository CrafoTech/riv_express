...
const company_trips_history_div = document.querySelector(
  ".company_trips_history_div",
);
const rider_trips_history_div = document.querySelector(
  ".rider_trips_history_div",
);
const company_notifications_div = document.querySelector(
  ".company_notifications_div",
);
const rider_notifications_div = document.querySelector(
  ".rider_notifications_div",
);

const company_tirps_history_card = document.querySelector(
  ".company_tirps_history_card",
);
const rider_tirps_history_card = document.querySelector(
  ".rider_tirps_history_card",
);

const rider_notification_cards = document.querySelector(
  ".rider_notification_cards",
);
const company_notification_cards = document.querySelector(
  ".company_notification_cards",
);
const rider_notifications_btn = document.querySelector(
  ".rider_notifications_btn",
);
const company_notifications_btn = document.querySelector(
  ".company_notifications_btn",
);
...
let cash_payment_company_check_div = document.querySelector(
  ".cash_payment_company_check_div",
);
let cash_payment_company_check_img = document.querySelector(
  "#cash_payment_company_check_img",
);

let company_gig_start_point_txt = document.getElementById(
  "company_gig_start_point_txt",
);
let company_gig_end_point_txt = document.getElementById(
  "company_gig_end_point_txt",
);
let company_gig_amount_charged_txt = document.getElementById(
  "company_gig_amount_charged_txt",
);

let rider_gig_start_point_txt = document.getElementById(
  "rider_gig_start_point_txt",
);
let rider_gig_end_point_txt = document.getElementById(
  "rider_gig_end_point_txt",
);
let rider_gig_amount_charged_txt = document.getElementById(
  "rider_gig_amount_charged_txt",
);
...
const select_company_work_btn = document.querySelector(
  ".select_company_work_btn",
);
const select_rider_work_btn = document.querySelector(".rider_work_btn");

const compny_work_history_btn = document.querySelector(
  ".compny_work_history_btn",
);
const rider_work_history_btn = document.querySelector(
  ".rider_work_history_btn",
);
...
async function getRidersData() {
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/getonerider_id/data/rider_data/${rider_ID}`,
    );
    const data = await res.json();
    riderArr = await data;
    console.log(
...
let loading_location_img_company = document.getElementById(
  "loading_location_img_company",
);
...
async function submitGigCreationFunc() {
  ...
  try {
    ...
    switch (work_kind_flag) {
      case "company":
        if (
          company_gig_start_point_txt.value.trim() &&
          company_gig_end_point_txt.value.trim() &&
          company_gig_amount_charged_txt.value.trim()
        ) {
          errorSignal = false;
          ...
          global_company_rider_trip_amount =
            company_gig_amount_charged_txt.value;
        } else {
          errorSignal = true;
          let errObj = {
      ...
      case "rider":
        if (
          rider_gig_start_point_txt.value.trim() &&
          rider_gig_end_point_txt.value.trim() &&
          rider_gig_amount_charged_txt.value.trim()
        ) {
    ...
    if (!errorSignal) {
      const res = await fetch(
        `${baseUrl}/createtrip/${dbname}/${cname}/${asset_ID}/${current_date}`,
        {
          method: "PATCH",
          headers: {
      ...
      const res2 = await fetch(
        `${baseUrl}/update_rider_collection/${dbname}/${cname}/${asset_ID}/${rider_ID}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
...
async function startGigFunc() {
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/starttrip/${dbname}/${cname}/${asset_ID}/${current_date}/${t_id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
async function finishingGigFuc() {
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/finishtrip/${dbname}/${cname}/${asset_ID}/${current_date}/${t_id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    switch (work_kind_flag) {
      case "company":
        company_work_btn.style.background = select_work_btn_background;
        company_work_btn.style.color = select_work_btn_color;
        company_work_btn.style.borderBottomColor =
          select_work_btn_bottom_border_color;
      ...
      case "rider":
        ...
        rider_work_btn.style.borderBottomColor =
          select_work_btn_bottom_border_color;
...
async function deleteGigFunc() {
  ...
  try {
    ...
    const res = await fetch(
      `${baseUrl}/deletetrip/${dbname}/${cname}/${asset_ID}/${current_date}/${t_id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
...
let company_trips_collected_amount_span = document.getElementById(
  "company_trips_collected_amount_span",
);
let company_total_trips_span = document.getElementById(
  "company_total_trips_span",
);
let rider_trips_collected_amount_span = document.getElementById(
  "rider_trips_collected_amount_span",
);
let rider_total_trips_span = document.getElementById("rider_total_trips_span");
let company_history_filter_date_inp = document.getElementById(
  "company_history_filter_date_inp",
);
let rider_history_filter_date_inp = document.getElementById(
  "rider_history_filter_date_inp",
);
...
async function getGenralNotifications() {
  try {
    ...
    console.error(
      `Oops - While getting General notifications: ${err.name}_${err.message}`,
    );
  }
}
      ...
      switch (data) {
        case true:
          ...
          localStorage.setItem(
            "logged_in_user",
            JSON.stringify(logged_in_user_obj),
          );
...
window.addEventListener("beforeinstallprompt", (e) => {
  // Prevent the browser from showing its own default prompt automatically
  // e.preventDefault();
  // Save the event so we can trigger it later
  deferredPrompt = e;
  // Make your custom install button visible
...
installBtn.addEventListener("click", async () => {
  ...
  // Show the native install prompt dialog box
  deferredPrompt.prompt();

  // Wait for the user to respond to the prompt (Accepted or Dismissed)
  const { outcome } = await deferredPrompt.userChoice;
  console.log(`User response to install prompt: ${outcome}`);

  // We used the prompt, so clear it out
  deferredPrompt = null;
  // Hide our custom button again
  installBtn.style.display = "none";
});

// Optional: Hide the button if the user has already installed the app
