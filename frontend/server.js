...
async function initConnection() {
    try {
        ...
        console.error(`Oops Error While Connectiong to the DB: ${err.name}_${err.message}`)
    }
}
...
app.get('/getonedoc_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const data = await db.collection(cname).find({_id: new ObjectId(id)}).toArray()
        return res.status(200).json(data)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Getting One Document By DOC ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.patch('/updateonedoc_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const rslt = await db.collection(cname).updateOne({_id: new ObjectId(id)},{$set: req.body})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating One Document By DOC ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.delete('/deleteonedoc_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Deleting One Document By DOC ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.get('/getoneasset_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Getting One Asset By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
// Getting One Asset By ASSET ID
app.get('/getonerider_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Getting One Rider By RIDER ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.patch('/updateone_rider_asset_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const rslt = await db.collection(cname).updateOne({asset_id:id},{$set: req.body})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating  Rider's Info By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.patch('/updateone_rider_rider_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const rslt = await db.collection(cname).updateOne({rid:id},{$set: req.body})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating  Rider's Info By RIDER ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
// Updating Riders' Data By RIDER ID || COLLECTIONS
app.patch('/updateone_rider_collections_rider_id/:dbname/:cname/:id', async (req,res) => {
    let {dbname,cname,id} = req.params
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const rslt = await db.collection(cname).updateOne({rid:id},{$set: {collections: req.body}})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating  Rider's COLLECTIONS By RIDER ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.delete('/deleteoneasset_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Deleting One Asset By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.delete('/deleteone_rider_rider_id/:dbname/:cname/:id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Deleting One Rider By RIDER ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
// Deleting ALL Riders By ASSET ID
app.delete('/deletemany_riders_asset_id/:dbname/:cname/:id', async (req,res) => {
    let {dbname,cname,id} = req.params
    try {
        const client = await initConnection();
        const db = await client.db(dbname);
        const rslt = await db.collection(cname).deleteMany({"asset.asset_id":id})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Deleting Many RIDERS By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.patch('/createcollection/:dbname/:cname/:id/:id_rid', async (req,res) => {
    ...
    try {
        ...
        const rslt = await db.collection(cname).updateOne({asset_id:id},{$set:{collections: data[0].collections}})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating  Collections Info By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
//Updating collections for A Rider
app.patch('/update_rider_collection/:dbname/:cname/:id/:rider_id', async (req,res) => {
    ...
    try {
        ...
        // data[0].collections.push(req.body)
        const rslt = await db.collection('rider_data').updateOne({rid:rider_id},{$set:{collections: newRiderCollections}})
        return res.status(200).json(rslt)
    } catch (err) {
        const serverErrObj = generateServerError(err,`Updating  Rider ${rider_id} Collections Info By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
// Creating a new trip
    ...
    try {
        ...
        switch (newCOllectionFlag) {
            case true:
                ...
                new_collection.amount_expected =  data[0].collections[len-1].balance_due
...
app.patch('/starttrip/:dbname/:cname/:id/:date/:t_id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Updating  Trip -${t_id}- status Info By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
// Finishing a trip
...
app.patch('/deletetrip/:dbname/:cname/:id/:date/:t_id', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Deleting  Trip -${t_id}- status Info By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
...
app.get('/getone_collec_cdate/:dbname/:cname/:id/:cdate', async (req,res) => {
    ...
    try {
        ...
        const serverErrObj = generateServerError(err,`Getting One Collection for One Asset By ASSET ID: ${id}`);
        return res.status(500).json(serverErrObj)
    }
})
